$(function() {
	...
	// Crash 2 varibles
	scrashCymbolAll = $('#SCrash');
	scrashCymbol = $('#SCrash-Cymbol');
	scrashAudio = $('#SCrash-Audio');

	// Crash timeline
	var scrashtl = new TimelineMax({
		paused: true
	});
	scrashtl.to(scrashCymbol, 0.1, {rotation: 8, transformOrigin: "50% 50%"})
	       .to(scrashCymbol,1.5, {rotation: 0, transformOrigin: "50% 50%", ease: Elastic.easeOut.config(2.5, 0.3)});

	// Crash play
	window.scrash = function() {
		scrashtl.restart();
		scrashtl.play();
		var scrashAudioEl = scrashAudio.get(0);
		scrashAudioEl.currentTime = 0;
		scrashAudioEl.play();
	}

	// Do the crash stuff when clicked/touched
	var clickTouchsCrashDone = false;
	scrashCymbolAll.on("touchstart click", function() {
		if(!clickTouchsCrashDone) {
			clickTouchsCrashDone = true;
			setTimeout(function() {
				clickTouchsCrashDone = false;
			}, 100);
			scrash();
			return false;
		}
...
// Ride  varibles
	rideCymbolAll = $('#Ride');
	rideCymbol = $('#Ride-Cymbol');
	rideAudio = $('#Ride-Audio');
	...
	window.ride = function() {
		ridetl.restart();
		ridetl.play();
		var rideAudioEl = rideAudio.get(0);
		rideAudioEl.currentTime = 0;
		rideAudioEl.play();
	}

	// Do the crash stuff when clicked/touched
	var clickTouchrideDone = false;
	rideCymbolAll.on("touchstart click", function() {
		if(!clickTouchrideDone) {
			clickTouchrideDone = true;
			setTimeout(function() {
				clickTouchrideDone = false;
			}, 100);
		ride();
			return false;
		}
...
// Hi-hat varibles
	...
	hiHattl.to([hiHatTop, hiHatBottom], 0.1, {rotation: 8, transformOrigin: "50% 50%"})
	       .to([hiHatTop, hiHatBottom], 1.6, {rotation: 0, transformOrigin: "50% 50%", ease: Elastic.easeOut.config(2.5, 0.3)});
...
// Hi-hat varibles
	ohiHatAll = $('#Open-Hi-Hat');
	ohiHatTop = $('#Open-Hi-Hat-Top');
	ohiHatBottom = $('#Open-Hi-Hat-Bottom');
	ohiHatClosedAudio = $('#Hi-Hat-Open-Audio');

	// Hi-hat timeline
	var ohiHattl = new TimelineMax({
		paused: true
	});
	ohiHattl.to([ohiHatTop, ohiHatBottom], 0.1, {rotation: 8, transformOrigin: "50% 50%"})
	       .to([ohiHatTop, ohiHatBottom], 1.6, {rotation: 0, transformOrigin: "50% 50%", ease: Elastic.easeOut.config(2.5, 0.3)});

	// Hi-hat play
	window.ohiHat = function() {
		ohiHattl.restart();
		ohiHattl.play();
		var ohiHatClosedAudioEl = ohiHatClosedAudio.get(0);
		ohiHatClosedAudioEl.currentTime = 0;
		ohiHatClosedAudioEl.play();
	}

	// Do the hi-hat stuff when clicked/touched
	var clickTouchoHitHatDone = false;
	ohiHatAll.on("touchstart click", function() {
		if(!clickTouchoHitHatDone) {
			clickTouchoHitHatDone = true;
			setTimeout(function() {
				clickTouchoHitHatDone = false;
			}, 100);
			ohiHat();
			return false;
		}
